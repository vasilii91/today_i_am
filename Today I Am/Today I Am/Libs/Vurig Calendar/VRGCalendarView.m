...
#define BACKGROUND_COLOR UIColorFromRGB(229, 231, 236)
...
-(void)reset {
    ...
    //NSLog(@"TEST %@",[components date]);
    
    [self updateSize];
    [self setNeedsDisplay];
//    [delegate calendarView:self switchedToMonth:[currentMonth month] targetHeight:self.calendarHeight animated:NO];
    [delegate calendarView:self switchedToMonth:[currentMonth month] year:[currentMonth year] targetHeight:self.calendarHeight animated:NO];
}

#pragma mark - Next & Previous
-(void)showNextMonth {
    if (isAnimating) return;
    if([currentMonth month]== [[NSDate date]month] && [currentMonth year]== [[NSDate date]year])
        return;
    ...
    if ([delegate respondsToSelector:@selector(calendarView:switchedToMonth:year:targetHeight:animated:)]) [delegate calendarView:self switchedToMonth:[currentMonth month] year:[currentMonth year] targetHeight:self.calendarHeight animated:YES];
...
-(float)calendarHeight {
    return kVRGCalendarViewTopBarHeight + [self numRows]*(kVRGCalendarViewDayHeight+2)+10;
}
...
{       
    ...
    CGRect rectArrowLeft = CGRectMake(10, 15, 34, 34);
    CGRect rectArrowRight = CGRectMake(276, 15, 34, 34);
    ...
    if (CGRectContainsPoint(rectArrowLeft, touchPoint)) {
        ...
        [self reset];
        if ((todayMonth!=currentMonthIndex) && [delegate respondsToSelector:@selector(calendarView:switchedToMonth:year:targetHeight:animated:)]) [delegate calendarView:self switchedToMonth:[currentMonth month] year:[currentMonth year] targetHeight:self.calendarHeight animated:NO];
    }
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    labelCurrentMonth.frameY = 25;
    labelCurrentMonth.backgroundColor=[UIColor clearColor];
    labelCurrentMonth.font = [UIFont fontWithName:[FontManager currentFontName] size:17];
    ...
    UIImage *header = [UIImage imageNamed:@"calendarHeader"];
    [header drawInRect:CGRectMake(0,0,self.frame.size.width,60)];
    
    UIImage *left = [UIImage imageNamed:@"calendarLeft"];
    [left drawInRect:CGRectMake(10, 17, 34, 34)];
    ...
    if([currentMonth month]== [[NSDate date]month] && [currentMonth year]== [[NSDate date]year])
    {
        ...
        UIImage *right = [UIImage imageNamed:@"calendarRight"];
        [right drawInRect:CGRectMake(276, 17, 34, 34)];
    }
    ...
    NSMutableArray *weekdays = [[NSMutableArray alloc] initWithArray:[dateFormatter weekdaySymbols]];
    ...
    for (int i =0; i<[weekdays count]; i++) {
        ...
        weekdayValue = [weekdayValue stringByPaddingToLength:3 withString:weekdayValue startingAtIndex:0];
        UIFont *font = [UIFont fontWithName:[FontManager currentFontName] size:11];
        [weekdayValue drawInRect:CGRectMake(i*(kVRGCalendarViewDayWidth+2), 80, kVRGCalendarViewDayWidth+2, 20) withFont:font lineBreakMode:UILineBreakModeClip alignment:UITextAlignmentCenter];
    }
    ...
    [[UIImage imageNamed:@"callendarBottom"] drawInRect:CGRectMake(-5,kVRGCalendarViewDayHeight*numRows+kVRGCalendarViewTopBarHeight+10,self.frame.size.width+10,13)];
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        if (i<firstWeekDay) { //previous month
            ...
            if([markedDates containsObject:[NSNumber numberWithInt:targetDate]])
            {
                CGRect rectangleGrid = CGRectMake(targetX,targetY,kVRGCalendarViewDayWidth+2,kVRGCalendarViewDayHeight+2);
                [[UIImage imageNamed:@"calendarSelectedDate"] drawInRect:rectangleGrid];
            }
        ...
        [date drawInRect:CGRectMake(targetX+1, targetY+10, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight) withFont:[UIFont fontWithName:FONT_NAME_TAHOMA size:17] lineBreakMode:UILineBreakModeClip alignment:UITextAlignmentCenter];
    }
...
-(id)init {
    ...
    if (self) {
        ...
        labelCurrentMonth.font = [UIFont fontWithName:@"MyriadPro-Regular" size:20];
